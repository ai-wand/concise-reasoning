...
import re 
def preprocess_box_response_for_qwen_prompt(sequence, answer, tokenizer, steps, **generate_kwargs):
    # breakpoint()
    ...
    # Tokenize the model output and apply additional reward penalty if tokens exceed 400 and episode is bigger than a limit
    if generate_kwargs['penalize_long_cot']:
        reward_increment = generate_kwargs['reward_high'] / (generate_kwargs['cot_threshold_high'] - generate_kwargs['cot_threshold_low'])
        if steps > generate_kwargs['step_threshold']:
            tokenized_output = tokenizer.encode(model_output, add_special_tokens=False)
            response_length = len(tokenized_output)
            if response_length > generate_kwargs['cot_threshold_low'] and response_length <= generate_kwargs['cot_threshold_high']:
                box_match += reward_increment * (response_length - generate_kwargs['cot_threshold_low'])
            if response_length > generate_kwargs['cot_threshold_high']:
                box_match += generate_kwargs['reward_high']
            print(f'response_length: {response_length}')
            print(f'box_match: {box_match}')
...
class NaiveExperienceMakerBOX(ABC):
    ...
    @torch.no_grad()
    def make_experience_list(self, all_prompts: Union[str, List[str]], all_answers:  Union[str, List[str]], steps, **generate_kwargs) -> List[Experience]:
        """
        Make a list of experience with the micro_rollout_batch_size.
        ...
        for idx, samples in enumerate(tqdm(
            ...
            experience = self.make_experience(samples, steps, **generate_kwargs)
...
class RemoteExperienceMakerBOX(NaiveExperienceMakerBOX):
    ...
    def make_experience_list(self, all_prompts: Union[str, List[str]], all_answers:  Union[str, List[str]], steps, **generate_kwargs) -> List[Experience]:
        ...
        experiences = super().make_experience_list(all_prompts, all_answers, steps, **generate_kwargs)
        if self.critic is not None:
            for experience in experiences:
    ...
    @torch.no_grad()
    def make_experience(self, samples: Samples, steps, **generate_kwargs) -> Experience:
        """
        Turn samples into experience by calculating logprobs, values, rewards, and kl divergence.
        """
        self.actor.eval()
        ...
        if not self.remote_rm_url:
            ...
            for query, answer in zip(queries, answers):
                query, box_match = preprocess_box_response_for_qwen_prompt(query, answer, self.tokenizer, steps, **generate_kwargs)
